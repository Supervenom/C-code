/*Created by Riccardo Paccagnella, Antonio Sterlicchio, Gabriele Vanoni, Rodrigo Principe, Francesco Vona, Alessandro Tatti */
...
int main (int argc,char *argv[])
{
	...
	while(!feof(ingresso))
	{
		useless=fscanf(ingresso, "%s", parola); /*prende ogni parola del testo*/
		parole[i]=strdup2(parola); /*copia la parola in memoria e salva la posizione nell'array parole*/
...
int contaParole (char* nomeFile)
{
	...
	while(!feof(f))
	{
		useless=fscanf(f, "%s", tmp); /*prende ogni parola del testo*/
		i++; /*continua a copiare tutte le parole del testo in memoria fino a quando finisce*/
	}
	fclose(f);
	return i;
